...
class Posting(Cog):
    ...
    async def post_FG_leaderboard(self, ctx):
        if isfile("./data/db/Full_Game_Leaderboard.txt"):
            await ctx.send("__**Full Game Leaderboard**__", file=File("./data/db/Full_Game_Leaderboard.txt"))
        else:
    ...
    async def post_IL_leaderboard(self, ctx):
        if isfile("./data/db/Individual_Levels_Leaderboard.txt"):
            await ctx.send("__**Individual Levels Leaderboard**__", file=File("./data/db/Individual_Levels_Leaderboard.txt"))
        else:
            await ctx.send("This leaderboard does not exist.")
    
    @command(name='leaderboard_ranking', aliases=['lb_ranking', 'ranking'])
    async def post_ranking_leaderboard(self, ctx):
        if isfile("./data/db/Ranking_Leaderboard.txt"):
            await ctx.send("__**Ranking Leaderboard**__", file=File("./data/db/Ranking_Leaderboard.txt"))
        else:
            await ctx.send("This leaderboard does not exist.")
